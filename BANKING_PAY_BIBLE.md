...
- Candidate Banking now reads at most 25 complete Ready rows per existing opaque server cursor. The main candidate list remains independently bounded at 100 candidates per page. Rollback-contained read-only measurements proved the larger candidate as three complete pages: 25 rows / 250,462 bytes, 25 rows / 333,692 bytes and 1 row / 31,308 bytes. Candidate-wide selection state and totals remain server-owned across the complete candidate scope; detail paging never turns a loaded page into the selection authority.
