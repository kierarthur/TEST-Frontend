...
- A selectable outer group exposes the complete server-owned `NONE`, `SOME` or `ALL` state. Its parent control selects or unselects the group's complete current eligible membership, not only the loaded detail page.
...
- Local proof before publication passed the complete backend application suite (1,105/1,105), the complete frontend unit suite (1,299/1,299), focused Worker/frontend contract tests, a rollback-contained first-use fixture, a 107-member grouped-paging fixture, a clean PostgreSQL 17 NEW-database replay and an exact generated-contract comparison whose only database changes were the two existing Candidate Ready reader definitions. Deployment and signed-in browser evidence must be appended before this entry is described as release-complete.
