...
- Executable Chromium proof uses the exact long NHS client-name shape and requires the rendered client label to remain within the Client cell, the Client cell to end no later than the Pay method cell begins, the pay-method badge to remain wholly inside its own cell and the label's two-line containment rule to be active. The same focused test continues through the existing expansion failure/retry, complete segment rendering, read-only refresh, close and parent-position preservation checks. Publication and real signed-in deployed-browser evidence must be appended before this item is release-complete.
