...
Sections 1-19 are the current controlling rules. Section 20 is an immutable chronological evidence trail: it records what an earlier release did, including behaviour later found to be wrong. An older Section 20 statement must never override a current rule in Sections 1-19 or a later explicit supersession. In particular, every earlier allowance for a display-only Candidate Banking outer row with a dash is superseded by section 6.3; preserving the historical words is audit evidence, not permission to restore that presentation.
...
### 6.3 Candidate Banking current-payable group projection

Candidate Banking is a control surface for the candidate's current Draft-relevant Ready scope. It is not a historical Timesheet ledger and it must not show an outer payment group merely because an older representative row remains in Workbench presentation storage.

- A `TIMESHEET` or `OVERPAYMENT` outer group may appear only when the current server-owned group facts prove at least one current selectable Ready member for that exact candidate, group kind and group key.
- A fully paid, superseded, historical, context-only or otherwise non-payable Timesheet with no current selectable Ready member is excluded from Candidate Banking. It must not appear with a dash, an empty expansion or a checkbox invented by the browser.
- A current single-row financial presentation may remain only through the established `ROW` contract and only while that row is itself current Ready authority.
- Candidate Banking must be a complete projection of current Draft-relevant Ready authority: every admitted current eligible group appears exactly once, and no historical, settled, superseded, context-only, non-current or otherwise ineligible group appears. A successful empty or partial-looking browser render is not proof of completeness; the response identities and counts must reconcile to the certified server scope.
- The outer group amount is the server-owned current outstanding amount ex VAT for that payment group. It is not the historical parent total, an inc-VAT total or an amount recalculated by the browser.
- Expanding the group returns only its current eligible members. Their current ex-VAT amounts must reconcile exactly to the server-owned outer group amount. A fully paid historical component must not reappear in the child table.
- Every admitted `TIMESHEET` or `OVERPAYMENT` outer group exposes the complete server-owned `NONE`, `SOME` or `ALL` state. Its parent control selects or unselects the group's complete current eligible membership, not only the loaded detail page. `SOME` is rendered as an indeterminate/half-tick state. An admitted grouped row must not fall back to a dash or an individual representative-row checkbox.
- A complete overpayment group is resolved from the exact accepted current row identity to its canonical `OVERPAYMENT` group kind and group key before display or mutation. Its expansion uses that same canonical group and shows all current recovery lines inside the bounded table. A legacy preview-row identifier may locate the accepted row for presentation compatibility, but must never become the selection owner.
- Candidate-wide selected counts and the Ready-to-pay headline remain server-owned across the complete candidate/session scope. They are never a sum of the visible outer page or expanded child page.
- Prior payments, active reservations and settled history continue to be handled by the existing economic and residual owners before this projection. Presentation code must not reinterpret them.

This projection does not change Draft preparation. `Create drafts` continues to consume the same exact selected, Draft-eligible Ready row identities and their canonical `amount_ex_vat`; neither the outer payment group nor its representative display row is a Draft input.

A deduction may be dependent on selected wages rather than independently selectable. For example, a loan or payroll-advance repayment can reduce what the candidate receives only when the selected wage payments provide enough money to deduct it. The user controls the wage payment; the established server headroom owner controls how much of the dependent repayment can follow it. A current overpayment recovery that the server publishes as its own selectable Ready group retains its own certified group control. The browser must not guess which kind of deduction it is from wording or amount.
...
The resulting recovery behaviour is mandatory: full remaining headroom retains the full permitted recovery in Ready; partial headroom retains only the permitted partial recovery amount; zero headroom removes the recovery from Candidate Banking/Ready and presents it once in Blocked for Pay as `Insufficient funds to deduct`. Re-selecting positive pay may promote the recovery back to Ready. All of these amounts and movements are server-owned and must settle in the same open modal revision.
...
- break;
- amount;
- where a real payment-method resolution exists, direction-specific labels rather than technical source/target wording: `Original PAYE amount`, `New Umbrella amount`, `Original PAYE rate`, `New Umbrella rate`, or the exact Umbrella-to-PAYE reverse.

A payment-method resolution is summarised once for the complete Timesheet/payment group; it is not repeated beneath every segment. When more than one rate applies (for example day, night, Saturday, Sunday, bank holiday or an additional rate), the expanded resolution shows one compact comparison row per real rate identity under the same group-level explanation. Suggested values remain the existing server-owned suggestion and resolution authority; the breakdown does not invent, average or replace rates, and the Original/New cards appear only when a real resolution exists.

Do not render a second new-method-only copy of a segment already represented by its Original/New comparison. Deduplication must use canonical identity, not visual similarity.
...
- Candidate Banking outer groups, representative rows, paging cursors and browser totals are presentation controls only and are never submitted as Draft items. Draft preparation continues to receive the exact canonical selected Ready row identities and their server-owned `amount_ex_vat` values.
...
11. Candidate Banking completeness -> every current eligible group appears exactly once, every outer amount equals its complete current ex-VAT membership, and no settled/historical/context-only group appears.
12. Complete grouped control -> parent NONE/SOME/ALL selection covers all current members across child pages; both Timesheet and overpayment groups bind to their exact canonical group, never a representative preview row.
13. Draft preflight after Candidate presentation changes -> the authoritative selected identity set, selected count, selected ex-VAT headline, `can_create_draft` and Draft-safety result reconcile without using any outer-group or browser-calculated value.
...
- Candidate Banking must keep the accepted server response immutable while preparing an outer payment group for the retained row renderer. A `TIMESHEET` or `OVERPAYMENT` group is admitted only when the server supplies a complete current group-fact tuple for the same presentation kind and key. A shallow temporary display copy may clear the representative row's display-only allocation exclusion only for that certified group, after which the complete-group binder must remove every representative preview-row identity and bind the control to the exact certified group kind and key. A grouped Timesheet or overpayment whose current selection-group tuple is absent is not Candidate Banking content and must be excluded by the server reader; the browser must not turn it into either a dash row or a selectable row. The existing `ROW` contract remains the only null-group exception.

## 20. Change record

### 1 September 2026 — Candidate Banking page retention and flicker-free authoritative refresh

- A real signed-in TEST reproduction proved that a user's unticked payment did not revert on the server. The apparent re-tick occurred because the later automatic Pay refresh discarded Candidate Banking's current outer-page anchor and silently reread page one. Page two contained the retained unticked groups, while page one's different visible groups were all selected; the unexpected page change therefore looked like a selection reversal.
- Automatic Candidate Banking refresh now reuses the accepted server-provided `page_anchor`. The server remains responsible for resolving that anchor against current payment-group membership and may return the nearest valid page only when genuine current membership removes the prior page. The browser does not calculate an offset, retain a stale cursor, re-sort a page or infer selection from visible rows.
- The Candidate Banking presenter continues to construct the complete refreshed table away from the visible DOM, apply every authoritative checkbox state, shape and control binding there, and then replace the visible table body once. This prevents an intermediate default checkbox state from painting. The same single adoption still publishes genuine server-owned changes to rows, amounts, deductions, recovery/headroom placement, totals and paging.
- Frontend commit `e85a53878ef56b9d14c3a38943ab0b2146bce709` owns the page-anchor correction, the single-replacement checkbox regression, the controlling section 13 rules and the coherent `20260901-r13` Banking asset marker. Focused integration, Candidate presenter, structure and cache tests passed 53/53; syntax and diff checks passed. The wider local frontend suite remained at its pre-existing baseline of 1,409 passing and the same two unrelated failures reproduced unchanged on the untouched preceding commit.
- App-Ready run `33507070719` passed the Office/UI seam and asset-policy job. Its separate Candidate/Client fixture suite passed 103/104 and repeated the same pre-existing desktop rate-client picker instability already present on the preceding frontend commit; the failure did not execute or reference a Banking Pay asset, route or test.
- GitHub Pages run `33507069746` deployed the exact commit successfully. The served TEST `20260901-r13` integration asset matched the committed asset after normalising GitHub Pages line endings and contained the current-page-anchor correction.
- Real signed-in deployed TEST acceptance opened Kier Arthur Candidate Banking on page two with 10 visible payment groups and `24 of 28 payment segments selected`. One exact selected group was unticked and settled at `23 of 28`; continuous 16 ms DOM sampling through the later automatic refresh proved that page two remained open and the target checkbox never visibly re-ticked or flickered. The same exact group was then reticked, the original page checkbox vector and `24 of 28` state were restored, and no Banking Pay request failed. No Draft, snooze, provider, payment, settlement or remittance action was performed.
- Policy X assessment: compliant. The change does not alter eligibility, selection ownership, ex-VAT amounts, recovery/headroom economics, Timesheet identity, the canonical Create Draft row contract, frozen Draft artefacts or any downstream financial lifecycle authority.

### 1 September 2026 — current payable groups, outstanding ex-VAT amounts and Draft-contract preservation

- The owner confirmed that Candidate Banking must contain only payments relevant to the current pay run. Historical, fully paid, superseded or context-only Timesheet parents with no current eligible Draft payment must not be shown merely to provide background information.
- This approved current-payable rule supersedes the earlier 31 August presentation policy which deliberately retained display-only outer Timesheet rows with a dash. The older change-record bullets remain evidence of what that release did; they are not authority to restore those rows after this correction.
- Read-only Miget TEST evidence separated three real shapes without changing data: one grouped Timesheet whose historical/representative display was £540.00 but whose complete current payable membership was three £150.00 ex-VAT segments (£450.00 total); one expense Timesheet whose historical parent was £4.00 but £3.00 had already settled, leaving one current £1.00 ex-VAT component; and one historical £17.39 Timesheet with no current selectable component or residual. The approved Candidate presentation is therefore £450.00 with three £150.00 children, £1.00 with one £1.00 child, and no outer row for the fully paid £17.39 item.
- The bounded v2 Candidate readers now compose the existing certified group-fact owner instead of trusting an arbitrary representative row amount. `TIMESHEET` and `OVERPAYMENT` outer rows require a matching current group-fact tuple; `ROW` remains the existing current single-row exception. Group detail requires the same current group identity and returns only current eligible members. An old group requested after it has left current Ready authority fails with the typed non-mutating `BANKING_PAY_V2_ITEM_NOT_CURRENT` result.
- The Worker and frontend response boundaries require a null selection-group tuple to belong only to a `ROW`, and require every non-null selection-group kind to match the presentation-group kind. They do not infer selectability, amounts or group ownership.
- Outer paging remains exactly 10 complete payment groups per full Candidate page. Inner paging remains bounded and independent. A 107-member synthetic Timesheet group occupies one outer position and is read through bounded child pages without omitting, duplicating or moving another outer group.
- The headline and candidate totals remain canonical server-selected ex-VAT scalars. Prior settlement and residual logic remains upstream and unchanged. `Create drafts` remains owned by the existing Draft preparation authority and consumes the same exact selected Ready row identities and `amount_ex_vat`; no group representative or browser-calculated amount enters the Draft contract.
- A dependent loan or advance recovery may follow selected wages under the existing headroom authority rather than expose a freely independent component control. A current overpayment recovery published as its own selectable group retains its certified group control. This distinction is server-owned and is not inferred from labels.
- Complete-group presentation uses the exact canonical group contract for both Timesheets and overpayments. Real deployed evidence found one retained overpayment row still using its legacy preview-row identity for the outer checkbox and expansion. The controller now resolves that accepted row to the certified `OVERPAYMENT` kind/key, binds one complete-group NONE/SOME/ALL control and reads the current recovery members through the same canonical group. The accepted response remains immutable and a true `ROW` retains its existing single-row authority.
- Local proof passed the complete backend application suite (1,105/1,105), the final complete frontend unit suite (1,302/1,302), focused Worker/frontend contract tests, a rollback-contained first-use fixture, a 107-member grouped-paging fixture, a clean PostgreSQL 17 NEW-database replay and an exact generated-contract comparison whose only database changes were the two existing Candidate Ready reader definitions.
- Backend TEST commit `50a0ded265b2b34ebbbb89036eb07380d71aa18e` was installed by protected managed Miget TEST UPGRADE run `33499184341`; its exact-head backend and database gates succeeded and normal TEST Worker version `973ee128-f1ca-4b02-9a05-6b12e23ee1a5` became active at 100%. Frontend commits `d1b51204dff26403da3581da8f8d1fb50c969927` and `aee5bcc47dc3fe4d34daddf8a5ae707cb5e69382` published the current-payable projection and complete overpayment-group bridge. Pages run `33502480780` and App-Ready run `33502482047` both succeeded at the final frontend asset commit.
- Exhaustive signed-in deployed TEST acceptance reconciled every current line rather than sampling the screen. The authoritative scope contained 31 selected eligible lines and selected headline `£450.00` ex VAT: Baljit had one Timesheet group containing three current £150.00 lines (£450.00), while Kier had 22 complete current groups containing 28 selected lines whose positive payments and current overpayment recoveries reconciled to £0.00. The three outer pages were exactly 10/10/2 groups; every group expanded to exactly its current certified members; every outer amount equalled the sum of those members' `amount_ex_vat`; there were no duplicate, missing, historical or ineligible identities. The settled 12 April £17.39 Timesheet was absent and the partially settled expense Timesheet exposed only its current £1.00 ex-VAT residual.
- The same accepted revision reported `can_create_draft=true` and Draft safety true for the exact 31-row selected identity set and £450.00 ex-VAT scalar. No group representative, presentation amount or browser sum entered that proof. No real Draft was created, so the acceptance did not create a financial artefact or invoke provider, payment, settlement, remittance or cancellation behaviour.
- A bounded reversible signed-in TEST mutation then proved the retained recovery behaviour on the deployed release. Kier began at 28 of 28 selected segments, 22 current Ready groups, 10 current recovery groups, £0.00 and Blocked count 5. Unticking one current one-segment positive Timesheet settled at 27 of 28 and `Some payments included`; at least one canonical recovery presentation changed under server authority while the recovery set remained Ready because partial headroom still existed. Reticking that exact group restored 28 of 28, every recovery signature, all Ready/Blocked counts, Kier £0.00 and the main £450.00 headline exactly. The test did not force zero headroom; the mandatory zero-headroom Ready-to-Blocked branch remains owned and tested by sections 6.2, 7, 7.1 and the server regression matrix.
- Desktop and 412 x 915 browser acceptance proved the bounded Candidate child, reachable `Close`, stable parent position, 10-group paging, current Timesheet expansion and current overpayment expansion. The observed candidate page transitions settled below one second in that run. Known unauthorised capability probes and the absent favicon remained unrelated browser noise; no Banking application request or adoption failure occurred.
- Policy X assessment: compliant. The correction changes current pre-Draft presentation/projection only. It changes no eligibility, selection economics, prior-payment ownership, residual calculation, recovery/headroom, rates, VAT, ERNI, PAYE/Umbrella treatment, Draft input, frozen post-Draft artifact, provider execution, payment, settlement, remittance or cancellation authority.

### 1 September 2026 — historical certified Candidate Banking group-checkbox bridge (superseded presentation scope)

- This entry remains the evidence record for the earlier checkbox bridge. Its allowance for null-group/context outer rows with dashes was superseded later on 1 September by the approved current-payable rule in section 6.3 and the preceding change entry. Those rows must now be excluded from Candidate Banking rather than retained.
...
### 31 August 2026 — historical outer payment-group presentation and retryable read-only adoption

- This entry records the 31 August release. Its then-approved rule that a non-selectable Timesheet representative remained visible without a checkbox is superseded by section 6.3. Current Candidate Banking excludes that outer row unless it has a complete current group-fact tuple or is a valid current `ROW`.
...
- At that release, a Timesheet outer representative with no selectable group remained visible with its breakdown and ordinary controls but without a selection checkbox. That historical behaviour must not be restored; the current reader excludes the row under section 6.3, while presentation still must never invent selection authority merely because a row is visible.
...
### 1 September 2026 — historical ordinary components are not signed recovery evidence

- A real TEST Workbench session failed before Draft creation while rebuilding one candidate's current payment scope. This was not a payment-calculation rejection and no Draft had been created. The deterministic first cause was `PAY_BATCH_SIGNED_NON_CHARGE_RECOVERY_EVIDENCE_INVALID` inside the pre-Draft source reconciliation stage.
- The affected historical frozen evidence legitimately contained more than one component with the same `TS_DAY` or `TS_TOTAL` key. Some were ordinary worked-time, rate or correction components; they were not signed non-charge recovery evidence. The helper `private.pay_batch_signed_non_charge_recovery_evidence_v1(jsonb)` counted every same-key component before proving the signed-recovery shape, so valid ordinary history could incorrectly trigger its strict one-component cardinality guard.
- Signed non-charge recovery classification must first require the complete pre-signature: `source_basis_json.component_fallback = WORKED_TIME_AMOUNT`, authoritative truth exactly zero, and authoritative baseline strictly negative. Cardinality is then applied only to the components that meet that pre-signature. One genuine signed component continues through all existing key, amount, physical-bucket, financial-revision, target-authority, conversion-context and sealed-evidence digest checks. No genuine signed component returns no signed-recovery evidence. Two or more genuine signed components remain an invalid ambiguity and fail closed.
- Ordinary components are never discarded, rewritten or reclassified by this correction. They remain part of their existing frozen financial evidence and continue through their established owners. The change only stops an ordinary same-key component from being counted as though it were a signed recovery component.
- The original job made seven short deterministic failed attempts with the same exact first cause. Its eighth delivered attempt later expired after its lease and cancellation grace, and the terminal job showed `DELIVERED_ATTEMPT_EXHAUSTED`. The lease outcome is later evidence, not the root cause. Diagnostics, user copy and future recovery audit tooling must preserve the earliest deterministic causal error and must not replace it with a later timeout, lease expiry or attempt-exhaustion label.
- After the corrected authority was installed, the user-approved current-authority refresh durably queued a replacement build. Its new `PREPARE_SCOPE`, `DEPENDENCY_CLOSURE`, `WORKSPACE_FACT`, `RECONCILE_EXECUTE` and `SOURCE_PUBLISH` stage jobs each completed successfully on their first attempt. The exact reconciliation stage that had previously failed completed in approximately 2.8 seconds. The browser's initial 20-second observation ended before the durable continuation began; subsequent stored evidence and a fresh browser open proved completion rather than assuming it from the queued response.
- Backend TEST commit `f5f09b140f4ac9188762e36baf9d2d14bd4c17a7` contains the full replacement repeatable `supabase/repeatable/01092026_1647_banking_pay_signed_recovery_classifier_v1.sql`, mandatory verifier `supabase/verification/01092026_1648_banking_pay_signed_recovery_classifier_verification.sql`, source regression and one-definition generated-contract update. The contract SHA-256 is `d21916d6b2e07965edf50c42d3aa9a752ba2a2ff1ca9a39ab91df970fd7e70af`.
- Local evidence passed the focused 10-test classifier suite, the complete backend suite 1,106/1,106, the new rollback-contained first-use verifier, the existing signed-recovery verifier, the full signed Draft runtime verifier, a clean PostgreSQL 17 NEW replay and exact generated-contract comparison. The first-use matrix proves: multiple ordinary same-key components return no signed evidence; ordinary history plus one genuine signed component returns the genuine signed result; and ordinary history plus two genuine signed components still fails with the established integrity error.
- Protected managed Miget TEST UPGRADE run `33530075121` succeeded on the exact commit. The current managed ledger is 208 migrations and 508 repeatables. Installed closure SHA-256 for the new repeatable is `81107b3bfea54b3b198c9e03fcb33e2dd0dceee2d1af83bbb3e3b8c4ef144542`; installed function definition SHA-256 is `06fd7386091293cb24797f5ddffc0f236d85c953058e1098e9b945c0e57b7b72`. Installed ownership, invoker security, fixed `pg_catalog` search path and execute ACLs match the repository contract.
- Real signed-in deployed TEST acceptance then opened Banking Pay with status `READY`, six of six candidate scopes terminally materialised, zero failed, pending, dirty or unknown candidates, zero queued/running/unresolved failed/dead jobs, `scope_reconcile_status = SUCCEEDED`, no Banking failure alert and an enabled `Create drafts` action. The accepted current payment list remained server-owned. No Create Draft, payment, provider submission, settlement, remittance, cancellation, Snooze or selection mutation was performed during this recovery proof.
- This correction does not prove every possible Workbench recovery or every possible Banking Pay/Create Draft permutation. The separate exhaustive Workbench self-recovery audit owns retry classification, lease interruption, cancellation acknowledgement, deterministic-error suppression, causal-error preservation and automatic recovery policy. The separate Banking Pay/Create Draft parity audit owns exact selected constituent and Draft-seed equality. Their verdicts remain independent and must be joined only by a shared certification bridge.
- Contradiction audit: no existing economic, selection, recovery/headroom, one-effective-section, Draft-input or Policy X rule is superseded. This entry narrows only the signed historical-evidence classifier and clarifies causal error reporting. Policy X assessment: compliant; no amount, VAT, ERNI, PAYE/Umbrella treatment, eligibility, selected membership, recovery allocation, Draft input, frozen post-Draft artefact, provider, settlement, remittance or cancellation authority changed.

### 2 September 2026 — Pay Filters shared Apply action availability

- Fresh signed-in TEST observation found that the Pay Filters child correctly prepared its existing filter/session-reset action but exposed no visible way to submit it. The generic Banking shell deliberately hides the shared modal footer, while `banking-pay-filters` depended on that footer for its existing Apply action. The problem was presentation/action reachability; it was not a Workbench selection, payment-policy, amount, eligibility, pay-channel, Draft, provider or database defect.
- The narrow frontend correction gives only the `banking-pay-filters` child the same explicit shared-footer exception already used by the Banking Snooze children and labels its existing primary action `Apply filters`. The main Banking Pay sheet continues to hide the shared footer. The established filter save owner, exact session-reset call, fail-closed new-session adoption check and previous-session preservation on failure are unchanged.
- Real deployed Playwright then proved the filter action visible, but also exposed that its existing Candidate and Client picker children inherited the same Banking-wide hidden-footer rule. Their row-selection owners worked, yet their existing shared `Apply` action was unreachable. The completed presentation correction therefore includes only `candidate-picker` and `client-picker` in the same footer exception. No picker search, selected identity, filter signature or session-reset owner changed.
- Local evidence passed JavaScript syntax, 27/27 focused Banking/modal/cache tests and the new exact source/CSS regression. The complete non-browser frontend suite passed 1,410/1,412; the same two unrelated failures reproduced on the untouched `e58e567f66ed8108a40e3c3e8388dbe33e0b0361` baseline at 1,409/1,411, proving this change added one passing test and no failure. Publication and a real new-session browser lifecycle remain required before this entry is release-complete.
- Policy X assessment: compliant. The correction exposes an already-existing pre-Draft user action and changes no selection authority, selected identity, amount, tax/VAT, PAYE/Umbrella rule, recovery/headroom, Draft input, frozen artifact, provider, payment, settlement, remittance or cancellation behaviour.
