...
- Publication and real-browser evidence must be added here before the correction is called released. Acceptance requires the deployed Kier Candidate Banking view to retain 22 payment groups paged 10/10/2, expand the previously failing group into its complete bounded detail, keep the mobile `Close` action reachable, preserve the parent modal position/state and show no raw internal code. No selection, Create Draft, provider, payment, settlement or remittance action is permitted for this read-only acceptance.
