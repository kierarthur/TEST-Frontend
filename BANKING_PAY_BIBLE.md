...
- Same-modal recovery routing after an accepted selection change is closed for the frontend selection-result path by commit `6ef2285868bcab8c59272aba8b62ddc6d321bbc6`. Ready and Blocked are fetched concurrently for one exact accepted session/version/progress/selection-epoch tuple, validated as a disjoint pair, and adopted once before the existing outer action owner performs one terminal render. A mismatched, partial, duplicate, stale, or failed pair is never partially published. The exact historical Kier browser fixture was not present in the 21 August TEST materialisation, so that named row was not recreated; executable tests cover Ready-to-Blocked and Blocked-to-Ready, and the deployed generic selection lifecycle proves the same live request/adoption path without modal reopen.
