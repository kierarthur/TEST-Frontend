...
test('Clear all Decisions fails closed unless the server confirms explicit empty selection', () => {
  ...
  assert.match(body, /clearResult\.server_selected_preview_row_ids\.length !== 0/);
  assert.match(body, /BANKING_PAY_CLEAR_DECISIONS_CONTRACT_INVALID/);
  assert.match(body, /markExplicitNoSelection\(\)/);
});
