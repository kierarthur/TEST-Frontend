...
test('loads the current main frontend asset through an explicit cache version', () => {
  const buildVersion = '20260728-invoice-v8-presentation-r1';
  ...
  const batchAsset = `invoice-batch-modal.js?v=${buildVersion}`;
  const asyncAsset = 'invoice-async-ui.js?v=20260728-invoice-async-v8-correction-r5';
  const stylesheetAsset = `invoice-batch-modal.css?v=${buildVersion}`;
  assert.match(
    html,
  ...
  assert.match(html, /invoice-batch-modal\.js\?v=20260728-invoice-v8-presentation-r1/);
