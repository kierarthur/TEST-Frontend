...
test('queued individual selections capture exact IDs and bind the latest accepted child at dispatch',async()=>{
 ...
 assert.equal(calls[1].expected_view_digest,'4'.repeat(64));assert.deepEqual(calls[1].open_ready,{cursor:'current_ready_anchor',limit:10});
...
test('queued complete-group selection binds server group identity and never loaded row IDs',async()=>{
 ...
 assert.equal(Object.hasOwn(calls[0],'preview_row_ids'),false);assert.deepEqual(calls[0].open_ready,{cursor:'current_ready_anchor',limit:10});
